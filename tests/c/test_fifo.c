...
#ifndef QHW_BAD_PLUGIN_PATH
#error "QHW_BAD_PLUGIN_PATH must be defined"
#endif
...
static int test_policy_replays_queued_tasks(void)
{
	qhw_sched_qpu_t *qpu = NULL;
	qhw_sched_t *sched = NULL;
	qhw_sched_assignment_t assignment;
...
static int test_policy_reset_replays_queued_tasks(void)
{
	qhw_sched_qpu_t *qpu = NULL;
	qhw_sched_t *sched = NULL;
	qhw_sched_assignment_t assignment;
...
static int test_bad_plugin_rejected(void)
{
	qhw_sched_qpu_t *qpu = NULL;
	qhw_sched_t *sched = NULL;

	CHECK(make_scheduler(&qpu, &sched) == 0);
	CHECK(qhw_sched_load_plugin(sched, QHW_BAD_PLUGIN_PATH) ==
		QHW_SCHED_ERR_PLUGIN);
...
int main(void)
{
	...
	CHECK(test_policy_replays_queued_tasks() == 0);
	CHECK(test_policy_reset_replays_queued_tasks() == 0);
	CHECK(test_cancel_removes_ready_task() == 0);
	CHECK(test_bad_plugin_rejected() == 0);
	return 0;
}
